...
const StudentResultsPage = () => {
  ...
  if (error) {
    return (
      <div className="p-6 max-w-4xl mx-auto">
        <div className="bg-red-100 border border-red-400 text-red-700 px-6 py-4 rounded-xl shadow">
          <h3 className="text-xl font-semibold mb-2">Error</h3>
          <p>{error}</p>
          <button
            onClick={() => window.location.reload()}
            className="mt-4 bg-blue-800 text-white px-5 py-2 rounded-lg hover:bg-blue-900 transition"
          >
            Retry
          </button>
        </div>
  ...
  return (
    ...
    >
      <h2 className="text-4xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-blue-800 to-blue-600 mb-8 text-center">
        My Results
      </h2>

      {results.length === 0 ? (
        <div className="text-center">
          <p className="text-slate-700 dark:text-slate-300 mb-4">No results found.</p>
        ...
        >
          {results.map((r, i) => {
            ...
            return (
              <motion.div
                key={r.resultId}
                className="bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-700 rounded-xl shadow-md p-6"
